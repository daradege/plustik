    ...
    async def check(event, *args):
        try:
            if isinstance(expected_text, str):
                return getattr(event, "text", None) == expected_text or getattr(event, "caption",
                                                                                None) == expected_text or getattr(event,
                                                                                                                  "data",
                                                                                                                  None) == expected_text
            else:
                return getattr(event, "text", None) in expected_text or getattr(event, "caption",
                                                                                None) in expected_text or getattr(event,
                                                                                                                  "data",
                                                                                                                  None) in expected_text
        except:
            return False
...
def startswith(expected_text: Union[str, List[str]]):
    ...
    async def check(event, *args):
        try:
            if isinstance(expected_text, str):
                return getattr(event, "text", "").startswith(expected_text) or getattr(event, "caption", "").startswith(
                    expected_text) or getattr(event, "data", "").startswith(expected_text)
            else:
                e_texts = [
                    getattr(event, "text", "").startswith(expected_textt) or getattr(event, "caption", "").startswith(
                        expected_textt) or getattr(event, "data", "").startswith(expected_textt) for expected_textt in
                    expected_text]
                for e in e_texts:
                    if e:
                        return True
...
def digit():
    ...
    async def check(event, *args):
        try:
            return getattr(event, "text", "").isdigit() or getattr(event, "caption", "").isdigit() or getattr(event,
                                                                                                              "data",
                                                                                                              "").isdigit()
        except:
            return False
